...
def send_group_share_my_music(gid, jumpUrl, musicUrl, title, musicType='custom', image=''):
    if musicType in ['qq', '163', 'xm']:
        return 404, {}
    if image:
        return send_group_msg(gid,
                              f'[CQ:music,type={musicType},url={jumpUrl},audio={musicUrl},title={title},image={image}]')
    else:
...
def send_msg(msgType, sid, msg, auto_escape=False):
    resp = ''
    if msgType == 'private':
        resp = send_cqhttp_request('/send_msg', {'message_type': 'private', 'user_id': sid, 'message': msg,
                                                 'auto_escape': auto_escape})
    elif msgType == 'group':
        resp = send_cqhttp_request('/send_msg', {'message_type': 'group', 'group_id': sid, 'message': msg,
                                                 'auto_escape': auto_escape})
    else:
...
def set_group_kick(gid, uid, reject_add_request=False):
    resp = send_cqhttp_request('/set_group_kick',
                               {'user_id': uid, 'group_id': gid, 'reject_add_request': reject_add_request})
    return parse_cqhttp_resp(resp)
...
def set_group_special_title(gid, uid, special_title=''):
    resp = send_cqhttp_request('/set_group_special_title',
                               {'group_id': gid, 'user_id': uid, 'special_title': special_title})
    return parse_cqhttp_resp(resp)
...
def set_group_add_request(flag, sub_type, approve=True, reason=''):
    if reason:
        resp = send_cqhttp_request('/set_group_add_request',
